...
export default function FlagCarousel() {
  ...
  // Keep pausedRef in sync with paused state so the RAF loop
  // can read the latest value without re-running the effect
  useEffect(() => {
  ...
  // Single long-lived RAF loop — never restarts, so posRef is never reset
  useEffect(() => {
  ...
  }, []); // empty — runs once, never restarts

  // Drag-to-scroll — works in iframes, sandboxed previews, and real apps
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (next >= SINGLE_SET) next -= SINGLE_SET;
      if (next < 0) next += SINGLE_SET;
      posRef.current = next;
      track.style.transform = `translateX(-${posRef.current}px)`;
    };
  ...
  return (
    <>
      <style>{`
        ...
        .fc-root:active { cursor: grabbing; }
          ...
          <div className="fc-track" ref={trackRef}>
            {repeated.map((country, i) => (
              <div
                key={`${country.code}-${i}`}
                className="fc-flag"
                onMouseEnter={() => setTooltip({ name: country.name })}
